...
import { RoomMessage } from '@dytesdk/web-core/types/roomNode/RoomNodeClient';
 ...
 * transcriptionDivId is the id of the div where you want the (noOfTranscriptionsToShow) transcriptions to be populated.
 * If no valid `transcriptionDivId` is provided, we will safely skip dom update.
 * noOfTranscriptionsToCache is the count of transcriptions you want to save in the cache (Default: 200).
...
async function init({
    ...
    ws.onmessage = async (event) => {
        ...
        if (data.type === 'message_response') {
            data.messages?.forEach((message: any) => {
                ...
                if (message.from?.id === meeting.self.id) {
        ...
        if (data.type === 'message' && Object.prototype.hasOwnProperty.call(data.message, 'punctuated')) {
            // console.log('Live transcript (less accurate): ', data.message.punctuated.transcript, data);
    ...
    meeting.on('roomMessage', async (
        ...
        if (type === 'audioTranscriptionMessage') {
            ...
            // Keep last noOfTranscriptionsToCache transcriptions only to not have a huge array of elements
            transcriptions = this.transcriptions.slice(noOfTranscriptionsToCache);
